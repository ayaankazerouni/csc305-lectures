...
We realise that the functionality offered by `sum` doesn't really depend on _which_ two numbers are being summed. Whatever they are (as long as they're `int`s here), the rest of the function remains the same. We don't actually care that the values are `10` and `3`. So we can instead use the names `a` and `b` to refer to two numbers that will have values _at runtime_, i.e., when the `sum` function is invoked.
